...
const iconMap: Record<string, { icon: React.ElementType; color: string }> = {
  ...
  Users: { icon: UsersIcon, color: '#4caf50' },
  Receipt: { icon: ReceiptIcon, color: '#fbc02d' }, // Amarelo
};

const getAvatarConfig = (imgAvatar: string) => {
  return iconMap[imgAvatar] || { icon: CurrencyDollarIcon, color: '#1976d2' }; // Fallback padrão
};
...
export function CardBudget({ tittle, diff, trend, sx, value, imgAvatar }: BudgetProps): React.JSX.Element {
  ...
  const { icon: IconComponent, color } = getAvatarConfig(imgAvatar);
